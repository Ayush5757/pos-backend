...
async function save_new_order_inventorie_customer(req, res) {
  ...
  try {
    ...
    if (!isExist) {
      ...
      const newOrder = new roomorderInventorieTableSchema({
        ...body,
      });
      await newOrder.save();
      req.io.to(body?.shopID).emit("newRoomOrderArrived");
      return res.status(200).json({ msg: "Room Order placed" });
    }
  } catch (error) {
...
async function save__order_to_existing_inventorie_customer(req, res) {
  const body = req.body;
  try {
    const existingOrder = await roomorderInventorieTableSchema.findOne({
      "shopID": body?.shopID,
      "customer.phone": body?.customer.phone,
      "room.roomID": body?.room.roomID,
      "room.room_name": body?.room.room_name,
      "room.room_type_id": body?.room.room_type_id
    });
    if (existingOrder) {
...
async function get_new_order_inventorie_customer(req, res) {
  ...
  try {
    const order_data = await roomorderInventorieTableSchema.find({
      shopID: body?.shopID,
    });
    if (!order_data) {
    ...
    return res
      .status(200)
      .json({ msg: "New Room Order Requests", target: order_data });
  } catch (error) {
    return res.status(400).json({ msg: error.message });
  }
}
...
async function chanege_status_and_add_to_inventrie_to_table(req, res) {
  ...
  try {
    if (!body.shopID && !body.roomID) {
      return res.status(400).json({ msg: "Missing shopID in the request" });
    }
    const customer_order =
      await roomorderInventorieTableSchema.findOneAndDelete({
        shopID: body.shopID,
    ...
    const existed_inventorie = await roomInventorieTableSchema.findOne({
      shopID: body.shopID,
      "room.roomID": body?.roomID,
      "customer.phone": body?.customer?.phone,
    });
    ...
    if (existed_inventorie) {
      ...
      const update = {
        $set: {
          "rooms.$[t].room_data.$[i].total": total_rupes,
        },
      };
      const arrayFilters = [
      ...
      return res.status(200).json({
        msg: "Order Updated, Shifted to inventory",
        target: orderData,
      });
    }

    orderData.order_status = 1;
    const inventories = new roomInventorieTableSchema({ ...orderData });
    const new_order = await inventories.save();
    const update = {
      $set: {
        "rooms.$[t].room_data.$[i].orderID": new_order?._id,
        "rooms.$[t].room_data.$[i].total": new_order?.total + ((new_order?.total * 18) / 100),
      },
    };
    const arrayFilters = [
      { "t.room_type_id": body?.room_type_id },
    ...
    return res.status(200).json({
      msg: "Order Accepted, Shifted to inventory",
      target: { ...orderData, order_id: new_order?._id },
    });
  } catch (error) {
...
async function delete_inventrie_to_table(req, res) {
  const body = req.body;
  ...
  try {
    ...
    const customer_order = await roomorderInventorieTableSchema.findOneAndDelete({
      shopID: body.shopID,
      "room.roomID": body?.roomID,
      "customer.phone": body?.customer?.phone,
      order_status: 3,
...
module.exports = {
  ...
  delete_inventrie_to_table:delete_inventrie_to_table,
